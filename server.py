...
# 1. Initialize the core app OUTSIDE everything so Vercel always finds it
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "free_forever_enterprise_secret_token_2026")

try:
    # 2. Attempt to load the Database and Enterprise logic
    from flask_sqlalchemy import SQLAlchemy

    # Force an in-memory SQLite fallback. Vercel blocks writing to standard files.
...
# ==============================================================================
# DIAGNOSTIC INTERCEPTOR (CATCHES VERCEL CRASHES)
# ==============================================================================
    ...
    def diagnostic_catcher(path):
        return f"""
        <div style="background:#111; color:#ff5555; padding:2rem; font-family:monospace; border-radius:10px; max-width: 900px; margin: 2rem auto;">
            <h2 style="color: #ff3333; margin-top: 0;">🚨 CRITICAL BOOT FAILURE INTERCEPTED</h2>
            <p style="color: #ccc;">The system successfully blocked Vercel from crashing, but the application failed to boot. Below is the exact Python traceback:</p>
            <pre style="background:#000; color:#00ff00; padding:1.5rem; overflow-x:auto; border: 1px solid #333; border-radius: 5px;">{error_trace}</pre>
            <p style="color: #aaa;">Copy this green text or take a screenshot and show it to me so I can fix it immediately.</p>
        </div>
        """
